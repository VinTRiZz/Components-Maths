...
void Point::trimNumber(std::string &buffer, const uint strSize) const
{
...
std::string Point::str(const uint strSize, const char delim) const
{
    if (strSize < 2) return "";
...
void calculateImmediate(Point &p1, Point &p2, const double radius)
{
...
void rotatePoint(Point &p, const Point & rotateCenter, const double rotateAngleInRad)
{
    Point bufferPoint = p;
...
double bezierBasis(long pointIndex, long numberOfPoints, double curvePos)
{
    //      n!
    // ---------- * t ^ i * (1 - t) ^ (n - i)
    // i!(n - i)!
...
std::string convertRectToString(const Point & leftDownCorner, const Point & rightTopCorner, const uint strSize, const char rectDelim, const char pointDelim)
{
    std::string result;
...
std::pair<Point, Point> convertRectFromString(const std::string &rectStr, const char rectDelim, const char pointDelim)
{
    Point p1, p2;
    if (!rectStr.size())
...
#ifdef QT_CORE_LIB
std::string convertRectToStringQ(const QRectF & rect, const uint strSize, const char rectDelim, const char pointDelim)
{
    std::string result;
...
QRectF convertRectFromStringQ(const std::string &rectStr, const char rectDelim, const char pointDelim)
{
